...
- Helper 未安装、等待批准、需要更新、不可用或注册失败时，设置页必须把修复区放在“快捷操作”页首屏顶部。
- 修复区必须使用明确的警告层级和主操作按钮；Helper 已启用后恢复为页底的普通管理区，避免持续抢占注意力。

### R7 — 锁屏授权

- 用户点击“锁定屏幕”且应用尚未获得辅助功能权限时，必须立即触发 macOS 原生辅助功能授权提示。
- 权限未授予前不得继续发送按键事件；界面应给出可操作的本地化反馈，而不是只透传 System Events 错误。
...
- [ ] Helper 需要处理时，修复入口位于快捷操作设置页顶部；启用后回到底部。
- [ ] 首次点击“锁定屏幕”会请求辅助功能权限，拒绝时不执行 AppleScript。
