...
- **锚定滑动 + 连发修复 + 手感重做合并**（三批交织提交）：右指 tipTap 锁死根因两连（陈旧 Pending 永不失效 + 序号取开局触点 ID）；anchoredSlide 六触点落地验证架构承诺；连续调节改真实位移比例（灵敏度 1.0=整边缘对整量程）；HUD 电平条化；静音自动解除；边缘三姿态放宽（companion reach）+ 成对判定哨兵；快捷键录制器；解码 decodeIfPresent 化拆除未来加字段炸弹；边缘单帧上限 12→11 + 钳制不变量测试。后续项：HUD 28Hz 重绘节流；协作规则：子代理越界先报备 main。
- **触控板界面收纳 + 全局快捷键**（8d69d3d + cae9541）：分区三 tab（预览订阅随 tab 停启）、编辑器高级折叠（非默认自动展开）；快捷键分区（Carbon 差分注册、冲突校验、失败不半注册）、nextDisplay 窗口动作、陈旧绑定 Picker 显式 tag 防静默重指。check 自修 4 处+补 2 服务路径测试，678 全绿。Carbon 已知界限：系统自占组合注册成功但不触发，无法程序化识别（文案中性化）。
