...
1. [ ] 读 research/ui-inventory.md、design.md；grep `SettingsPane` 全部使用点（含深链、持久化、菜单项）列成清单存任务 notes。
2. [ ] 新 SettingsPane 枚举（9 case + 分组 + migrating(rawValue:) 映射）；侧边栏分组渲染（组标题小字 + 分区行）。
3. [ ] 建 5 个新分区视图文件（MenuBar/Panel/Calendar/Power/General SettingsView），内容从现视图**平移组装**（不重写控件逻辑）；设置窗口类型迁入 SettingsWindowViews.swift。
4. [ ] 「关于」瘦身；删除 概览/外观/iCloud/语言/日期与时间 独立分区；OverviewSettingsView.swift 拆解删除。
5. [ ] 逐项核对 research/ui-inventory.md 第二节分布表：每个设置项标注新归属，漏项为零。build+test。
