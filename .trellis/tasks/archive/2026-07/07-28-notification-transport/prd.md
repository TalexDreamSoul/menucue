...
- Fan out leased outbox events to all enabled channels concurrently, isolate outcomes, and acknowledge per-event/per-channel delivery status with the exact lease ID so stale workers cannot overwrite a reclaimed delivery.
...
- [ ] Keychain create/read/update/delete and missing/corrupt/locked/access-denied behavior are covered; tests assert every query's accessibility, non-synchronizable, and namespace attributes, plus update-first duplicate-add recovery.
- [ ] HTTPS-to-HTTP, cross-host, loop, and excessive redirects are rejected before any credential-bearing resend; response bytes are streamed and cancelled at the hard cap.
- [ ] Claim/outcome contracts preserve lease identity, and mixed fan-out/retry behavior is deterministic under fake clock/HTTP/outbox interfaces.
