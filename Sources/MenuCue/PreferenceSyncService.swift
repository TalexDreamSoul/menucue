...
enum PortableSettingField: String, CaseIterable, Codable, Hashable {
    ...
    case appearanceMode
    /// Safe to carry across machines: the policy falls back to the fastest rate on
    /// anything without a battery, so a desktop inherits the thresholds harmlessly.
    case metricsSampling
}
...
enum PortableSettingValue: Codable, Equatable {
    ...
    case metricsSampling(MetricsSamplingSettings)
}
...
struct PortableSettingEnvelope: Codable, Equatable {
    ...
    var isCompatible: Bool {
        ...
        case (.menuBarFormat, .menuBarFormat),
             ...
             (.metricsSampling, .metricsSampling):
            return true
        default:
            return false
        }
...
extension AppSettings {
    func portableValue(for field: PortableSettingField) -> PortableSettingValue {
        ...
        case .metricsSampling: return .metricsSampling(metricsSampling)
        }
    }
    ...
    mutating func applyPortableValue(
        ...
        case let (.appearanceMode, .appearanceMode(mode)):
            appearanceMode = mode
        case let (.metricsSampling, .metricsSampling(sampling)):
            // Normalize on the way in: a peer running an older or newer build could
            // send thresholds outside this build's ranges.
            metricsSampling = sampling.normalized
        default:
            return false
        }
        return true
