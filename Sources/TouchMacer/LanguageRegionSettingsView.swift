...
struct LanguageRegionSettingsView: View {
  ...
  private var appLanguageSection: some View {
    SettingsGroup(spacing: 12) {
      Text("TouchMacer Language")
        .font(.headline)

      Picker("TouchMacer Language", selection: $pendingLanguage) {
      ...
      Text("Changing the app language relaunches TouchMacer. It does not change the macOS language.")
        .font(.caption)
